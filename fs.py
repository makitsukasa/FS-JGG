...
class FS:
	...
	def alternate(self):
		...
		self.avg_history[self.eval_count] = np.average([i.fitness for i in self.population])
	...
	def until_stuck(self, max_eval_count):
		while self.eval_count < max_eval_count:
			self.alternate()
			if self.is_stucked():
				return True
...
if __name__ == '__main__':
	n = 20
	ga = FS(1e-4, n, 6 * n, n + 1, 6 * n, lambda x: np.sum((x * 10.24 - 5.12) ** 2))
