...
class FSJGG(object):
	...
	def choose_population_to_jgg_not_replace(self):
	...
	def choose_population_to_jgg_replace_rand_parents_by_elites(self, elites_count = None):
		...
		not_parents = pop[self.npar:]
		ret = not_parents[:]
		parents = pop[:self.npar]
		np.random.shuffle(parents)
		ret.extend(parents[:self.npar - elites_count])
		elites = self.fs.population[:]
		elites.sort(key = lambda i: i.fitness)
		ret.extend(elites[:elites_count])
		return ret

	def choose_population_to_jgg_replace_losed_parents_by_elites(self, elites_count = None):
		if elites_count is None:
			elites_count = self.npar
		pop = self.init_population[:]
		pop.sort(key = lambda i: i.fitness)
	...
	def choose_population_to_jgg_replace_rand_by_elites(self, elites_count = None):
		if elites_count is None:
			elites_count = self.npar
		pop = self.init_population[:]
		np.random.shuffle(pop)
		ret = pop[elites_count:]
		elites = self.fs.population[:]
		elites.sort(key = lambda i: i.fitness)
		ret.extend(elites[:elites_count])
		print(len(ret))
...
if __name__ == '__main__':
	n = 20
	ga = FSJGG(1e-2, n, 6 * n, n + 1, 6 * n, lambda x: np.sum((x * 10.24 - 5.12) ** 2))

	ga.choose_population_to_jgg = lambda :\
		ga.choose_population_to_jgg_replace_rand_parents_by_elites(7)
